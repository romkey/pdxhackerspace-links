...
class ThingsShowTest < ApplicationSystemTestCase
  ...
  test "show layout places description beside photo with grouped details below" do
    ...
    assert_selector ".detail-group .h-section-label", text: /Identifiers/i
    ...
    details_position = page.evaluate_script <<~JS
      Array.from(document.querySelectorAll('.prose, .h-section-label')).findIndex((el) => el.textContent.match(/Identifiers/i))
    JS
